...
//   const grammar = grammars.find((g) => g.names.includes(lang))
//   const language = languages.find((l) => l.scopeName === grammar.scopeName)
//   const children = language.embeddedLangs || []
...
async function fetchGrammars(api, lang) {
  const r = await fetch(`${api}/grammars?lang=${lang}`)
  const json = await r.json()
  if (!json) {
    throw new Error(`Grammars for ${lang} not found`)
  }
  return json
}

function getRegistry(api, lang) {
  ...
  if (!promises[key]) {
    promises[key] = fetchGrammars(api, lang)
      .then((grammars) => loadGrammars(registry, grammars))
      .then((grammars) => {
        grammars.forEach((grammar) => {
          grammar.names.forEach((name) => {
            cache["grammar-" + name] = { data: registry }
          })
        })
      })
      .catch((e) => {
...
export function load(api, lang, themeLabel) {
  ...
  const registry = getRegistry(api, lang)
  if (registry.promise) {
